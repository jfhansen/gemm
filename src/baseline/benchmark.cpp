...
static void BM_serial_gemm_power_of_two(benchmark::State &s) {
...
// Serial GeMM with matrix dimensions not being power of two.
static void BM_serial_gemm(benchmark::State &s) {
    // Fetch matrix dimension
    size_t N = ( 1 << s.range(0) ) + 9;
...
static void BM_parallel_gemm(benchmark::State &s) {
    ...
    for (auto _ : s) {
        ...
        for (size_t i = 0; i < num_threads; i++) {
            ...
            threads.emplace_back(parallel_gemm, A, B, C, N, start_row, end_row);
        }
        assert(end_row == N);
...
BENCHMARK(BM_serial_gemm_power_of_two)->DenseRange(8,10)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_parallel_gemm)->DenseRange(8,10)->Unit(benchmark::kMillisecond);
